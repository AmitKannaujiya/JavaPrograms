...
//        System.out.println(count);
    ...
    static boolean checkPrime(long no) {
        for(long i=2;i*i<no;i++){ if(no%i==0){return false;}  } return true;
    }
...
//    }
    ...
    public int maxSubArray(int[] nums) { int ans=Integer.MIN_VALUE; int cSu=Integer.MIN_VALUE;

    for(int i=0;i<nums.length;i++) { cSu=Math.max(nums[i], nums[i]+cSu); ans=Math.max(ans, cSu);}
    return ans;
    }
